...
from nextcord.ext.commands import Context, command, Bot, Cog
...
class Stats(Cog):
    """The cog containing the commands to check the stats of the counters."""
    ...
    async def id(self, ctx: Interaction, user: Member):
        """Gives a user's id without pinging them"""
        await ctx.send(f"{user.id}")

    @command(name="user", aliases=["u"])
    async def user_command(self, ctx: Context, member: Member | None = None):
        ...
        for bot_id in bot_collection:
            post = find_user_stats(user.id, bot_id)
            if post is not None:
                correct = Decimal(post.correct)
                wrong = Decimal(post.wrong)
                total = correct + wrong
                daily = post.daily
                streak = post.streak
                high = post.high
                # total_saves = post.get("total_saves", 3)
                if total == 0:
                    rate = 0
                else:
                    rate = (correct / total * 100).quantize(Decimal("10.000"))
                # current_saves = post["current_saves"]
                ...
                if daily != 0:
                    msg += f"\nDaily: {daily:,}"
                # msg += "\n`c!user`"
                embedVar.add_field(name=mode_list[f"{bot_id}"], value=msg)

        await ctx.send(embed=embedVar)

    @commands.command(name="currentscore", aliases=["cs"])
    async def currentscore_command(
        self,
        ctx: Context,
        ...
        if mode == "og":
            counter_cursor = (
                og_collection.find(
                    {"streak": {"$gte": 1}}, {"name": 1, "streak": 1, "_id": 0}
                )
                .sort("streak", -1)
                .skip(i)
                .limit(10)
        ...
        async def currentscore_slash(
            self,
            ctx: Interaction,
            mode: str = SlashOption(
                description="Leaderboard mode", choices=["og", "classic", "numselli"]
            ),  #'abc'
            page: int = SlashOption(
                description="The page number of the leaderboard", default=1
            ),
        ):
            """Shows the streak currentscores"""
            msg = ""
    ...
    async def leaderboard_command(
        ...
        if mode == "og":
            counter_cursor = (
                og_collection.find(
                    {"high": {"$gte": 1}}, {"name": 1, "high": 1, "_id": 0}
                )
                .sort("high", -1)
                .skip(i)
                .limit(10)
    ...
    async def leaderboard_slash(
        ...
        ctx: Interaction,
        mode: str = SlashOption(
            description="Leaderboard type", choices=["og", "classic", "numselli"]
        ),  #'abc'
        page: int = SlashOption(
            description="The page number of the leaderboard", default=1
        ),
        ...
        if mode == "og":
            ...
            while msg == "":
                ...
                for counter in counter_cursor:
                    ...
                    msg += f"{i}. {name} - {high}\n"
                if msg == "":
                    counter_num = og_collection.count_documents({"high": {"$gte": 1}})
                    page = int(counter_num / 10)
        elif mode == "classic":
            title_msg = "Highest streaks for classic counting"
            while msg == "":
    ...
    async def leaderboard(self, ctx: Union[Context, Interaction], page: int, mode: str):
        """The function that handles generation of the leaderboard."""
        i = (page - 1) * 10
        msg = ""
        if mode == "og":
            counter_cursor = (
    ...
    async def rankup_cmd(self, ctx: Context, member: Member | None = None):
        ...
        og_post: Optional[OGCounter] = og_collection.find_one(
            {"_id": f"{user.id}"}, {"correct": 1, "wrong": 1}
        )
...
#     #             await ctx.send(
#     #                 f"Can't transfer streaks. Check if revelant stats are there."
#     #             )
#     #         else:
...
#     """The object for controlling the view of Leaderboard"""
...
#     async def classic_btn(self, button: Button, interaction: Interaction):
...
#             content="You have cancelled the transfer.", embed=None, view=self
#         )
...
#         self.og_alt: Optional[OGCounter] = og_collection.find_one({"_id": self.alt.id})
...
#             if self.og is True and (
#                 "alt" not in self.og_alt or self.og_alt["alt"] != self.main.id
#             ):
...
#                 og_collection.update_one(
#                     {"_id": self.alt.id},
#                     {"$unset": {"alt": 1}, "$set": {"streak": 0, "high": 0}},
#                 )

#         if self.classic_alt is not None and self.classic_main is not None:
#             if self.classic is True and (
#                 "alt" not in self.classic_alt or self.classic_alt["alt"] != self.main.id
#             ):
...
#             if self.beta is True and (
#                 "alt" not in self.beta_alt or self.beta_alt["alt"] != self.main.id
#             ):
...
#             if self.num is True and (
#                 "alt" not in self.num_alt or self.num_alt["alt"] != self.main.id
#             ):
